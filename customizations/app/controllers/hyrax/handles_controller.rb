module Hyrax
    class HandlesController < ApplicationController
      ...
      def handle 
        resp = Hyrax::SolrService.get("id: #{params[:id]}")
        solr_hash = resp.dig("response", "docs", 0)
        @handle = solr_hash["handle_tesim"].first
      end
      ...
      def redirect
        @handle = handle
        #redirect_to :controller => 'handles', :action => "show", :handle => @handle
        redirect_to "/handle/#{ENV.fetch('HANDLE_PREFIX', '10822')}/#{@handle}"
      end 
